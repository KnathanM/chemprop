...
class MixedComponentMPNN(MPNN):
    def __init__(
        self,
        message_passing: MixedComponentMessagePassing,
        agg: Aggregation,
        predictor: Predictor,
        batch_norm: bool = True,
        metrics: Iterable[Metric] | None = None,
        ...
        self.message_passing: MixedComponentMessagePassing

    def fingerprint(
        self,
        bmgs_mixmpgs: tuple[Iterable[BatchMolGraph], tuple[Tensor, ...]],
        V_ds: Iterable[Tensor],
        X_f: Tensor | None = None,
    ) -> Tensor:
        bmgs, mixmpgs = bmgs_mixmpgs

        H_vs: list[Tensor] = self.message_passing(bmgs, V_ds)
        Hs = [self.agg(H_v, bmg.batch) for H_v, bmg in zip(H_vs, bmgs)]

        # n: num. components in group, b: num. mols in batch, d: out dim. of message passing
        Hs = [
            torch.einsum("bn,nbd->bd", mixmpg, torch.stack([Hs[idx] for idx in mpgm]))
            # print(mixmpg.shape, torch.stack([Hs[idx] for idx in mpgm]).shape)
            for mpgm, mixmpg in zip(self.message_passing.mpms, mixmpgs)
        ]
        H = torch.cat(Hs, 1)
        H = self.bn(H)
    ...
    def load_from_checkpoint(
        ...
        print("Load checkpoint not yet implemented")
